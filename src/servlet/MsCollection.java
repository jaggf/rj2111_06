...
@WebServlet("/MsCollection")
public class MsCollection extends HttpServlet {
	@Serial
		...
		switch (method) {
			...
			case "moreDelete":
				try {
					...
					Boolean boo = false;
					for (String ides : ids) {
						String sqls = "DELETE FROM ms WHERE  num= " + ides;
						stmt.executeUpdate(sqls);
						boo = true;
					}
					if (boo) {
						ms.setBackNews("删除成功，单击<a href='ms/managems.jsp'><font color='red'>返回</font></a>面试管理页面");
					} else {
						ms.setBackNews("删除失败，单击<a href='ms/managems.jsp'><font color='red'>返回</font></a>面试管理页面");
					}
					stmt.close();
					conn.close();
				} catch (ClassNotFoundException | SQLException e) {
				...
				RequestDispatcher dispatchersss = request.getRequestDispatcher("ms/msshow.jsp");
