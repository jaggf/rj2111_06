...
@WebServlet("/KaoqinCollection")
public class KaoqinCollection extends HttpServlet {
	@Serial
		...
		switch (method) {
			...
			case "search":
				...
				try {
					...
					request.getRequestDispatcher("kaoqin/kaoqinCheck.jsp").forward(request, response);
					rs.close();
					stmt.close();
					conn.close();
			...
			case "moreDelete":
				try {
					...
					Boolean boo = false;
					for (String ides : ids) {
						String sqls = "DELETE FROM kaoqin WHERE  num= " + ides;
				...
				RequestDispatcher dispatchersss = request.getRequestDispatcher("kaoqin/kaoqinCheck.jsp");
