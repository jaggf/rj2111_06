...
@WebServlet("/SalaryCollection")
public class SalaryCollection extends HttpServlet {
	@Serial
		...
		switch (method) {
			...
			case "search":
				...
				try {
					...
					if (!eid.isEmpty()) {
						sql += " AND eid LIKE '%" + eid + "%'";
					}
					if (!eusername.isEmpty()) {
						sql += " AND eusername LIKE '%" + eusername + "%'";
					}
					if (!year.isEmpty()) {
						sql += " AND year LIKE '%" + year + "%'";
					}
					if (!month.isEmpty()) {
						sql += " AND month LIKE '%" + month + "%'";
					}
					Statement stmt = conn.createStatement();
					ResultSet rs = stmt.executeQuery(sql);
					request.setAttribute("sa", rs);
					request.getRequestDispatcher("salary/checkSalary.jsp").forward(request, response);
					rs.close();
					stmt.close();
					conn.close();
			...
			case "searchUser":
				...
				try {
					...
					String password = "root";
					Connection conn = DriverManager.getConnection(url, user, password);
					String sqlU = "SELECT * FROM salary WHERE 1=1";
					if (!idU.isEmpty()) {
			...
			case "moreDelete":
				try {
					...
					Boolean boo = false;
					for (String ides : ids) {
						String sqls = "DELETE FROM salary WHERE  num= " + ides;
				...
				RequestDispatcher dispatchersss = request.getRequestDispatcher("salary/manage_salary.jsp");
