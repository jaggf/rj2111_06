...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-3">
            <form action="${pageContext.request.contextPath}/MegsCollection?method=upload" method="post"
        ...
        <div class="col-md-6">
            <form action="MegDownLoad" method="post">
                <button type="submit" class="btn btn-primary" style="height: 38px;" title="下载">
    ...
    <div class="row">
        <div class="col">
            <%
                try {
                    String strSql = "SELECT id, title, content, publish_time, order_num, time FROM notice ORDER BY publish_time DESC, order_num DESC;";
                    ResultSet rss = bd.getRs(strSql);
            %>
            <form action="${pageContext.request.contextPath}/MegsCollection?method=moreDelete" method="post">
                <button style="height: 38px;" title="新增" type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#myModal">
                    <%-- 新增--%>
                        <i class="fa fa-plus" aria-hidden="true" /></i>
                </button>&nbsp;&nbsp;<button type="submit" class="btn btn-danger" title="批量删除">
                <%--批量删除--%>
                <i class="fa fa-list" aria-hidden="true"/></i> <i class="fa fa-remove" aria-hidden="true"/></i>
            </button>
                <table class="table table-striped table-bordered">
                    <thead>
                    <tr>
                        <th style="text-align: left" style="text-align: center"><input type="checkbox" id="selectAll">
                        </th>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="btn-group">
                                ...
                                <%--                            <a href="${pageContext.request.contextPath}/ModEmployee1?id=<%=rs.getString(1) %>&method=update" class="btn btn-success" target="main_ifram">修改</a>--%>
                                <!-- 修改 -->
                                ...
                                <!-- 删除按钮 -->
                                <a title="<%=schu%>" href="${pageContext.request.contextPath}/MegsCollection?id=<%=rss.getString(1) %>&method=delete"
                                   class="btn btn-danger"> <i class="fa fa-remove" aria-hidden="true"/></i> </a>
                            </div>
                        </td>
                    </tr>
                    <% }
            ...
            <p>
            <form name="form1" method="post" action="megmager.jsp">
                <div class="text-center">
                    第<%=p %>页 共<%=intpagecount %>页&nbsp;&nbsp;
                    <%
                        if (p > 1) {
                    %>
                    <a href="megmager.jsp?page=1" class="btn btn-secondary">第一页</a>
                    <a href="megmager.jsp?page=<%=p-1 %>" class="btn btn-secondary">上一页</a>&nbsp;&nbsp;
                    <%
                        }
                        if (p < intpagecount) {
                    %>
                    <a href="megmager.jsp?page=<%=p+1 %>" class="btn btn-secondary">下一页</a>
                    <a href="megmager.jsp?page=<%=intpagecount %>" class="btn btn-secondary">最后一页</a>&nbsp;&nbsp;
                    <%
                        }
                    %>
